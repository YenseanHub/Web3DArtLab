...
onMounted(() => {
  AInstance.get(`/config/saplingConfig.json`).then((res) => {
    examleList = []
    let titleList = []
    Object.keys(res.data).forEach((ele, index) => {
      ...
      Object.keys(res.data[ele]).forEach((subEle) => {
        ...
        examleList[index].list.push({
          imgUrl: `/thumbnail/saplings/${val}.jpg`,
          cardName: subEle,
          valName: val
        })
        if (saplingId == val) {
...
function RedirectHtml(name) {
  const src = `/saplings/${name}.html`
  const iframeView = document.getElementById('sapling-container')
  if (null != iframeView) {
    iframeView.src = src
...
<template>
  <el-container class="examples-container">
    ...
    <iframe id="sapling-container" class="saplings-iframe-container"></iframe>
    <footer-mark />
  </el-container>
