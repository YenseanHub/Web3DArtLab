...
import { getUnitStr, getLengthValueByUnit } from './UnitConversionUtils.js'
import { unitRange, showPrecisionValue } from './DecimalPrecisionUtils.js'
...
export default class Measure {
  ...
  constructor(renderer, scene, camera, controls, mode = MeasureMode.Distance, settings) {
    this.isCompleted = false // whether measure operation is completed or not
    this.mode = null

    this.spriteMaterial = null
    this.raycaster = null
    this.mouseMoved = false
    this.pointMarkers = [] // used for measure distance and area
    this.polyline = null // the line user draws while measuring distance
    this.faces = null // the faces user draws while measuring area
    this.curve = null // the arc curve to indicate the angle in degree
    this.tempPointMarker = null // used to store temporary Points
    this.tempLine = null // used to store temporary line, which is useful for drawing line/area/angle as mouse moves
    this.tempLabel = null // used to store temporary label as mouse moves
    this.labels = [] // stores labels, so that we can clear them up when necessary
    this.pointArray = []
    this.lastClickTime // save the last click time, in order to detect double click event
  ...
  initPointMarkerMaterial() {
    const markerTexture = new THREE.TextureLoader().load('/addons/measure/circle.png')
  ...
  complete() {
    ...
    if (this.mode === MeasureMode.Area && this.polyline) {
      if (count > 2) {
        ...
        const label = `${showPrecisionValue(
          getLengthValueByUnit(area, unitRange['Unit from file'], this.settings.unit, 2).value,
          this.settings.decimalPrecision
        )} ${this.getUnitString()}`
  ...
  mousemove = (e) => {
    ...
    if (this.pointArray.length > 0) {
      ...
      if (this.mode === MeasureMode.Distance) {
        ...
        const label = `${showPrecisionValue(
          getLengthValueByUnit(dist, unitRange['Unit from file'], this.settings.unit).value,
          this.settings.decimalPrecision
